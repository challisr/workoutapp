(function () {
  ...
  function boot() {
    ...
    TOKEN = token;
    ROLE = (role === "coach" || role === "family") ? role : "athlete";
    const savedTab = localStorage.getItem("kicker_last_tab");
    ...
    app.innerHTML = `
      <div class="setup-screen">
        <div class="setup-box">
          ...
          <p>Track workouts, notes, and progress — share view/edit access with a coach or family.</p>
  ...
  // ---------- shell ----------
  function render() {
    const roleLabel = ROLE === "coach" ? "Coach view" : ROLE === "family" ? "Family view" : "Athlete view";
    const pc = pendingCount();
    app.innerHTML = `
      <div class="header">
        <div class="header-row">
          <div class="logo-badge"><img src="/wildcat-logo.png" alt="Plano Wildcats" /></div>
          <div>
            <h1>${esc(STATE.meta.athleteName)}'s Kicking Plan <span class="role-badge">${roleLabel}</span></h1>
            <div class="sub">Plano Wildcats <span id="saving-indicator" style="opacity:0; transition:opacity .2s;"> · saving…</span></div>
          </div>
        </div>
      </div>
    ...
    document.querySelectorAll(".tab").forEach((btn) => {
      btn.addEventListener("click", () => {
        TAB = btn.dataset.tab;
        localStorage.setItem("kicker_last_tab", TAB);
        render();
      });
    });
  ...
  function pendingActionsHtml(e) {
    ...
    return `<div class="pending-actions">
      <span style="font-size:12px;color:var(--muted);">Pending approval</span>
    </div>`;
  }
  ...
  function renderWeek(el) {
    ...
    let html = `
      <div class="card">
        <div class="day-header-row">
          ...
          ${canEdit ? `<button class="edit-toggle" id="toggle-edit">${editBtnLabel}</button>` : ""}
        </div>
        ...
        ${ROLE === "family" ? `<p style="font-size:12px;color:var(--muted);margin:6px 0 0;">Family view is read-only.</p>` : ""}
      </div>
    `;
    ...
    STATE.days.forEach((day) => {
      html += `<div class="card" data-day="${day.id}">
        ...
        ${canEdit && EDIT_MODE ? (ADD_FORM_OPEN === day.id ? addExerciseForm(day.id) : `<button class="btn btn-outline btn-block" data-add-day="${day.id}" style="margin-top:8px;">+ Add Exercise</button>`) : ""}
      </div>`;
    });
  ...
  function renderTeam(el) {
    ...
    const familyUrl = shareUrl("family");
    el.innerHTML = `
      <div class="card">
      ...
      <div class="card">
        ...
        <span class="link-label">Coach link (can suggest changes)</span>
        <div class="link-box">${esc(coachUrl)}</div>
        <button class="btn btn-outline btn-sm" data-copy="${esc(coachUrl)}">Copy Link</button>
        <span class="link-label">Family link (view only)</span>
        <div class="link-box">${esc(familyUrl)}</div>
        <button class="btn btn-outline btn-sm" data-copy="${esc(familyUrl)}">Copy Link</button>
      </div>
      <div class="card">
